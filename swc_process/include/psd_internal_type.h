...
struct Pose4f {
  ...
  Pose4f() = default;
  Pose4f(float px, float py, float pz, float pyaw)
...
//=============================================================================
enum class SlotParkable : uint8_t { Null = 0x00, Pass = 0x01, Fail = 0x02 };
...
struct StateMachineInput {
  ...
  bool shouldClearData() const {
    ...
    return status == ApaStatus::Off || status == ApaStatus::Standby ||
           status == ApaStatus::Finished || status == ApaStatus::Error;
  }
};
...
struct MapParkingSlot {
  ...
  SlotType getSlotType() const { return static_cast<SlotType>(psType); }
};
...
/// Grid map - from apa::fus::EmapGridmap (simplified, full grid data not
/// copied)
struct GridMapInput {
  Header header;
  uint64_t timestamp = 0;
...
struct ParkingSlotOutput {
  ...
  void clear() { *this = ParkingSlotOutput{}; }
};
