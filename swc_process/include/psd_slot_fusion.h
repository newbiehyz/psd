...
private:
    ...
    void ProcSelectTargetSlot(
        const PsdFusionInput& input,
        InternalSlotList& slot_list);

    /// Step 6: Update display status for all slots
    void ProcUpdateDisplayStatus(
        const PsdFusionInput& input,
        InternalSlotList& slot_list);
    ...
    static float CalcPointToLineDistance(const Point2f& point, const Point2f& lineA, const Point2f& lineB);
