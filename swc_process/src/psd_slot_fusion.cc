...
void PsdSlotFusion::Process(const PsdFusionInput& input,
  ...
  bool has_changed =
      (std::fabs(dx) > kStillThreshold || std::fabs(dy) > kStillThreshold ||
...
void PsdSlotFusion::ResetByStatus(const PsdFusionInput& input,
                                  InternalSlotList& slot_list) {
...
void PsdSlotFusion::ConvertMapInfo(const PsdFusionInput& input,
  ...
  for (size_t i = 0; i < input.mapInfo.parkingSlotCount; ++i) {
    ...
    slot.occupancy = map_slot.isOccupied ? SlotOccupancyStatus::Occupied
...
void PsdSlotFusion::ConvertUssSlots(const PsdFusionInput& input,
                                    InternalSlotList& slot_list) {
  ...
  for (size_t i = 0; i < Limits::kMaxUssSlots; ++i) {
    ...
    if (!merged) {
      ...
      for (int k = 0; k < 4; ++k) {
        slot.localCorners[k] =
            Point2f(static_cast<float>(uss_slot.pointABCD[k].x),
...
void PsdSlotFusion::ProcObstacles(const PsdFusionInput& input,
                                  InternalSlotList& slot_list) {
  ...
  for (size_t i = 0; i < Limits::kMaxObstacleCount; ++i) {
    ...
    for (size_t j = 0; j < slot_list.count; ++j) {
      ...
      // 计算障碍物到车位中心的距离
      float distance =
          std::hypot(obs_pt.x - slot_center.x, obs_pt.y - slot_center.y);
    ...
    if (obs.obsType == ObstacleType::WheelStop) {
      ...
      if (nearest_slot.type == SlotType::Perpendicular ||
        ...
        if (dist_to_AB > dist_to_CD) {
          nearest_slot.obstacleLocation =
              InSlotObstacleLocation::Center;  // 靠近CD边（底部）
          // 限位块在底部，不影响泊入
        } else if (dist_to_AB < dist_to_CD) {
          nearest_slot.obstacleLocation =
              InSlotObstacleLocation::Center;  // 靠近AB边（入口）
          // 限位块在入口附近，标记为占用
        ...
        if (dist_to_BC > dist_to_AD) {
          nearest_slot.obstacleLocation =
              InSlotObstacleLocation::Left;  // 靠近AD边
        } else if (dist_to_BC < dist_to_AD) {
          nearest_slot.obstacleLocation =
              InSlotObstacleLocation::Right;  // 靠近BC边
        } else {
...
void PsdSlotFusion::ProcSelectTargetSlot(const PsdFusionInput& input,
                                         InternalSlotList& slot_list) {
  uint8_t status = input.stateMachine.apaStatusReq;
  ...
  if (final_selected_id_ == 0 && is_vehicle_still_) {
    if (!available_slots.empty()) {
      ...
      for (size_t i = 0; i < available_slots.size() &&
                         i < static_cast<size_t>(kMaxRecommendNum);
           ++i) {
  ...
  if (status == static_cast<uint8_t>(ApaStatus::Guidance) &&
      has_saved_target_slot_) {
...
void PsdSlotFusion::ProcUpdateDisplayStatus(const PsdFusionInput& input,
                                            InternalSlotList& slot_list) {
  for (size_t i = 0; i < slot_list.count; ++i) {
    auto& slot = slot_list.slots[i];
...
void PsdSlotFusion::ConvertToOutput(const PsdFusionInput& input,
  ...
  output.slotList.header.sequence =
  ...
  for (size_t i = 0; i < slot_list.count && i < Limits::kMaxSlotCount; ++i) {
    ...
    for (int j = 0; j < 4; ++j) {
      dst.slotCorners[j] =
          Point3f(src.localCorners[j].x, src.localCorners[j].y, 0.0f);
    }
  ...
  if (target && target->hasStopperInSlot) {
    for (size_t i = 0; i < Limits::kMaxStopperCount; ++i) {
      ...
      output.slotStopper.limitBarY[i] =
          static_cast<int16_t>(target->stopperPoints[i].y);
    }
  }
}
...
SlotType PsdSlotFusion::ConvertMapSlotType(uint32_t map_type) {
  switch (map_type) {
    case 0:
      return SlotType::Perpendicular;
    case 1:
      return SlotType::Parallel;
    case 2:
      return SlotType::Oblique;
    case 3:
      return SlotType::ReverseOblique;
    case 4:
      return SlotType::UserDefined;
    default:
      return SlotType::Perpendicular;
  }
}
...
Point2f PsdSlotFusion::TransformBodyToWorld(const Point2f& pt_body,
...
Point2f PsdSlotFusion::TransformWorldToBody(const Point2f& pt_world,
                                            const VehiclePoseInput& pose) {
  float theta = pose.yaw * static_cast<float>(M_PI) / 180.0f;
  float cos_t = std::cos(theta);
  float sin_t = std::sin(theta);
...
void PsdSlotFusion::AdjustCornerOrder(SlotCorners& corners) {
  ...
  auto signed_area = [&](int i0, int i1, int i2, int i3) -> float {
    const Point2f* p[4] = {&corners[i0], &corners[i1], &corners[i2],
                           &corners[i3]};
    float area2 = 0.0f;
    for (int i = 0; i < 4; ++i) {
      const Point2f& a = *p[i];
  ...
  auto cross = [](const Point2f& a, const Point2f& b,
                  const Point2f& c) -> float {
    // (b - a) x (c - a)
    return (b.x - a.x) * (c.y - a.y) - (b.y - a.y) * (c.x - a.x);
  };

  auto segments_intersect = [&](const Point2f& p1, const Point2f& p2,
                                const Point2f& p3, const Point2f& p4) -> bool {
  ...
  enum class SlotSide { UNKNOWN, LEFT, RIGHT };
...
bool PsdSlotFusion::PointInQuad(const Point2f& point,
                                const SlotCorners& corners) {
  Point2f A = corners[0];
  Point2f B = corners[1];
  Point2f D = corners[3];
...
float PsdSlotFusion::CalcPointToLineDistance(const Point2f& point,
                                             const Point2f& lineA,
                                             const Point2f& lineB) {
  float numerator =
      std::fabs((lineB.y - lineA.y) * point.x + (lineA.x - lineB.x) * point.y +
                lineB.x * lineA.y - lineA.x * lineB.y);

  float denominator = std::sqrt((lineB.y - lineA.y) * (lineB.y - lineA.y) +
