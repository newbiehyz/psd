...
void PsdInputConverter::ConvertStateMachine(
    const apa::status::Output2ALL& input, StateMachineInput& internal_input) {
...
void PsdInputConverter::ConvertVehiclePose(const apa::loc::Vehicle_Pose& input,
...
void PsdInputConverter::ConvertMapInfo(const apa::loc::MapInfo& input,
...
void PsdInputConverter::ConvertUssSlots(const uss::UssSlotList& input,
...
void PsdInputConverter::ConvertGridMap(const apa::fus::EmapGridmap& input,
  ...
  internal_input.vagueCenterPoint.yaw =
      input.VagueEmapGrid.vaguecenterPoint.yaw;

  // Precise center point
  internal_input.preciseCenterPoint.x =
      input.PreciseEmapGrid.PrecisecenterPoint.x;
  internal_input.preciseCenterPoint.y =
      input.PreciseEmapGrid.PrecisecenterPoint.y;
  internal_input.preciseCenterPoint.z =
      input.PreciseEmapGrid.PrecisecenterPoint.z;
  internal_input.preciseCenterPoint.yaw =
      input.PreciseEmapGrid.PrecisecenterPoint.yaw;
}
...
void PsdInputConverter::ConvertObstacles(const apa::fus::PkEmapObs& input,
  ...
  for (size_t i = 0; i < Limits::kMaxObstacleCount; ++i) {
    ...
    dst.motionInfo.centerUncertainty =
        ConvertPoint3f(src.obsMotionInfo.centerUncertainty);
    dst.motionInfo.velocity = ConvertPoint3f(src.obsMotionInfo.velocity);
    dst.motionInfo.velocityUncertainty =
        ConvertPoint3f(src.obsMotionInfo.velocityUncertainty);
    dst.motionInfo.acceleration =
        ConvertPoint3f(src.obsMotionInfo.acceleration);
    dst.motionInfo.accelerationUncertainty =
        ConvertPoint3f(src.obsMotionInfo.accelerationUncertainty);
    dst.motionInfo.velocityHeading = src.obsMotionInfo.velocityHeading;
    dst.motionInfo.velocityHeadingUncertainty =
        src.obsMotionInfo.velocityHeadingUncertainty;
    dst.motionInfo.velocityHeadingRate = src.obsMotionInfo.velocityHeadingRate;
    dst.motionInfo.velocityHeadingRateUncertainty =
        src.obsMotionInfo.velocityHeadingRateUncertainty;

    // Trajectory
    dst.trajectory.motionStatus =
        static_cast<MotionStatus>(src.obsTrajectory.motionStatus);
    dst.trajectory.confidence = src.obsTrajectory.confidence;
    dst.trajectory.pointCount = 0;
    for (size_t j = 0; j < src.obsTrajectory.points.size() &&
                       j < Limits::kMaxTrajectoryPoints;
         ++j) {
      const auto& pt = src.obsTrajectory.points[j];
      dst.trajectory.points[dst.trajectory.pointCount].deltaTNs = pt.deltaTNs;
      dst.trajectory.points[dst.trajectory.pointCount].center =
          ConvertPoint3f(pt.center);
      dst.trajectory.points[dst.trajectory.pointCount].direction =
          ConvertPoint3f(pt.direction);
    ...
    for (size_t j = 0;
         j < src.obsBdBox.size() && j < Limits::kMaxBoundingBoxPoints; ++j) {
...
void PsdInputConverter::ConvertPlanningDecision(
  ...
  internal_input.parkingSpace.chamferP0.aisleSidePoint =
      ConvertPoint2f(input.parkingSpace.chamferP0.aisleSidePoint);
  internal_input.parkingSpace.chamferP0.slotSidePoint =
      ConvertPoint2f(input.parkingSpace.chamferP0.slotSidePoint);
  internal_input.parkingSpace.chamferP5.aisleSidePoint =
      ConvertPoint2f(input.parkingSpace.chamferP5.aisleSidePoint);
  internal_input.parkingSpace.chamferP5.slotSidePoint =
      ConvertPoint2f(input.parkingSpace.chamferP5.slotSidePoint);
  ...
  internal_input.targetPos.finalTargetPos.yaw =
      input.targetPos.finalTargetPos.yaw;
  internal_input.targetPos.finalTargetPos.curv =
      input.targetPos.finalTargetPos.curv;
  ...
  internal_input.targetPos.stageTargetPos.yaw =
      input.targetPos.stageTargetPos.yaw;
  internal_input.targetPos.stageTargetPos.curv =
      input.targetPos.stageTargetPos.curv;
}
...
void PsdInputConverter::ConvertSelectSlot(
    const apa::uic::SelectParkingSlot& input, SelectSlotInput& internal_input) {
  internal_input.slotId = input.slot_id;
  internal_input.parkingDirection =
      static_cast<ParkingDirection>(input.parking_direction);
...
void PsdInputConverter::ConvertCustomSlot(
  ...
  internal_input.parkingSlotType =
      static_cast<uint8_t>(input.parking_slot_type);
  internal_input.slotSurfaceType =
      static_cast<uint8_t>(input.slot_surface_type);
...
void PsdInputConverter::ConvertAllInputs(
    ...
    const apa::loc::MapInfo& map_info, const uss::UssSlotList& uss_slots,
    const apa::fus::EmapGridmap& gridmap, const apa::fus::PkEmapObs& emap_obs,
    const apa::planning::DecPlan2Psd& parkable_slot,
    const apa::planning::Plan2Status& pnc_status, uint64_t current_timestamp_us,
